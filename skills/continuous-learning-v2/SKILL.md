...
| Feature | v2.0 | v2.1 |
|---------|------|------|
| Storage | Global (`${XDG_DATA_HOME:-~/.local/share}/ecc-homunculus/`) | Project-scoped (`${XDG_DATA_HOME:-~/.local/share}/ecc-homunculus/projects/<hash>/`) |
...
**On OpenCode**, observation capture is automatic: the `ecc-learning` plugin
(`~/.config/opencode/plugins/ecc-learning.ts`) registers deterministic
`tool.execute.before` / `tool.execute.after` hooks that append observations to
the same `ecc-homunculus` store in exactly the same JSONL schema as `observe.sh`.
No settings or hook registration is required.

Instinct **analysis** is model-driven on OpenCode: when the model notices
recurring patterns or user corrections (or the plugin's idle prompt fires), it
follows this skill to read recent observations and write/update instinct YAML
files. There is no background observer agent on OpenCode.

**Claude Code only** — installed as a plugin: no extra `settings.json` hook
block is required. Claude Code v2.1+ auto-loads the plugin `hooks/hooks.json`,
and `observe.sh` is already registered there.
...
**On OpenCode**, run the equivalent `instinct-cli.py` commands from this skill's
...
(global) and `<homunculus>/projects/<hash>/instincts/personal/` (per project).
...
Edit `config.json` to control the background observer (Claude Code only):
...
|-----|---------|-------------|
| `observer.enabled` | `false` | Enable the background observer agent (Claude Code) |
| `observer.run_interval_minutes` | `5` | How often the observer analyzes observations |
| `observer.min_observations_to_analyze` | `20` | Minimum observations before analysis runs |

**On OpenCode** there is no background observer process. Instinct analysis is
done by the model following this skill. The `ecc-learning` plugin can also
prompt the model to run it on `session.idle` when enough new observations
accumulate — tune with env vars on the plugin:

| Env var | Default | Description |
|---------|---------|-------------|
| `ECC_OBSERVER_AUTO_EVOLVE` | `1` | `0` disables the idle-time instinct evolution prompt |
| `ECC_OBSERVER_EVOLVE_MIN_OBS` | `10` | New observations required before an idle prompt fires |
| `ECC_OBSERVER_EVOLVE_COOLDOWN_MIN` | `10` | Minutes between idle prompts |
| `ECC_OBSERVE` | `1` | `0` disables observation capture entirely |
| `ECC_HOOKS_ENABLED` | `true` | `false` disables all ECC hooks |
