...
Do not wait for the user to type `/graphify`. If `graphify-out/graph.json` exists, treat architecture and “where/how does X” questions as graph queries first (`graphify query`, `path`, `explain`, or `GRAPH_REPORT.md`). After code edits in-session, run `graphify update .` (AST-only). Slash command is optional.
