...
async function main() {
  ...
  await test('accepts a benign cursor param on tools/list (newer MCP protocol)', async () => {
    await withClient(async client => {
      const tools = await client.listToolsRaw({ cursor: 'abc' });
      assert.ok(Array.isArray(tools.tools));
      assert.strictEqual(tools.nextCursor, undefined);
      const names = tools.tools.map(tool => tool.name).sort();
      assert.deepStrictEqual(names, ['memory_doctor', 'memory_read', 'memory_save', 'memory_search']);
    });
  });
